...
const SidePanel = ({
  ...
  const [panelOpen, setPanelOpen] = useState(isOpen || activeTabIndexProp !== null);
  ...
  const style = { ...styleMap[openStatus][side], ...baseStyle };

  const ActiveComponent = tabs[activeTabIndex].content;

  useEffect(() => {
    if (panelOpen && swiper && activeTabIndex !== swiper.activeIndex) {
      swiper.slideTo(activeTabIndex, 500);
    }
  }, [panelOpen, swiper, activeTabIndex]);
  ...
  const updatePanelOpen = useCallback((newPanelOpen) => {
    setPanelOpen((prevPanelOpen) => {
      const panelOpenValue = typeof newPanelOpen === 'boolean' ? newPanelOpen : !prevPanelOpen;
      if (panelOpenValue) {
        setHasBeenOpened(true);
      }
      return panelOpenValue;
    });
  }, []);

  const updateActiveTabIndex = useCallback((index) => {
  ...
  tabs: PropTypes.oneOfType([
    PropTypes.arrayOf(
      PropTypes.shape({
        ...
        isOpen: PropTypes.bool, // Agrega esta línea
      })
    ),
  ]),
  isOpen: PropTypes.bool, // Agrega esta línea
