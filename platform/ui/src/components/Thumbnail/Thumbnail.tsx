  ...
  return (
    ...
    >
      <div ref={drag}>
        <div
          className={classnames(
            'flex flex-1 items-center justify-center rounded-md bg-black text-base text-white overflow-hidden min-h-28',
            isActive
              ? 'border-2 border-primary-light'
              : 'border border-secondary-light group-focus:border-blue-300 hover:border-blue-300'
          )}
          style={contenedorImagen}
        >
          {imageSrc ? (
            <img
              src={imageSrc}
              alt={imageAltText}
              className="object-none min-h-28"
            />
          ) : (
        ...
        <div className="flex flex-row items-center flex-1 text-base text-blue-300">
          <div className="mr-4">
            <span className="font-bold text-primary-main w-3 mr-2">{'S:'}</span>
            {seriesNumber}
          </div>
